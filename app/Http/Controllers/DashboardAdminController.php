...
class DashboardAdminController extends Controller
{
    ...
    public function updateSetting(Request $request)
    {
        ...
        // ✅ Fire notification
        Notification::settingEvent('updated', $setting);

        // 📝 Add to audit logs
        AuditLog::create([
            'user_id'    => Auth::id(),
            'event'      => 'setting_update', // 👈 consistent event name
            'ip_address' => request()->ip(),
            'user_agent' => request()->userAgent(),
            'context'    => [
                'company_name'     => $setting->company_name,
                'logo'             => $setting->logo ? basename($setting->logo) : null,
                'background_image' => $setting->background_image ? basename($setting->background_image) : null,
            ],
        ]);

 ...
 */
class YTDMaterialSummaryWorksheet implements FromArray, WithHeadings, WithEvents, WithCustomStartCell, WithStyles, WithCharts, WithTitle
{
    // Class properties
    ...
    {
        return 'L' . $this->line .' '. $this->year . ' YTD SUMMARY ' ;
    }
    ...
    private function addPTDLineEfficiency(Worksheet $sheet): void
    {
        ...
        $categories = [
            'C' => 'J', // Preforms % Rejects (col H)
            'D' => 'O', // Caps % Rejects (col O)
            'E' => 'T', // OPP Labels % Rejects (col T)
            'F' => 'Y', // LDPE Shrinkfilm % Rejects (col Y)
        ];
    ...
    // === Material Efficiency (Average of C–F) ===
    $sheet->setCellValue("G{$row}", "=AVERAGE(C{$row}:F{$row})");
    ...
    private function prepareData(): array
    {
        ...
        for ($m = 1; $m <= 12; $m++) {
            ...
            $outputFormula = "=INDEX('L{$this->line} P{$m}'!F1:F200, MATCH(\"PTD\", 'L{$this->line} P{$m}'!E1:E200, 0))";
            $fgUsage = "=INDEX('L{$this->line} P{$m}'!H1:H200, MATCH(\"PTD\", 'L{$this->line} P{$m}'!E1:E200, 0))";

            $preformsrej = "=INDEX('L{$this->line} P{$m}'!I1:I200, MATCH(\"PTD\", 'L{$this->line} P{$m}'!E1:E200, 0))";
            $preformsqa = "=INDEX('L{$this->line} P{$m}'!J1:J200, MATCH(\"PTD\", 'L{$this->line} P{$m}'!E1:E200, 0))";
            $preformsrejper = "=IFERROR(VALUE(INDEX('L{$this->line} P{$m}'!K1:K200, MATCH(\"PTD\", 'L{$this->line} P{$m}'!E1:E200, 0))),0)";


            $capsrej = "=INDEX('L{$this->line} P{$m}'!N1:N200, MATCH(\"PTD\", 'L{$this->line} P{$m}'!E1:E200, 0))";
            $capsqa = "=INDEX('L{$this->line} P{$m}'!O1:O200, MATCH(\"PTD\", 'L{$this->line} P{$m}'!E1:E200, 0))";
            $capsrejper = "=IFERROR(VALUE(INDEX('L{$this->line} P{$m}'!P1:P200, MATCH(\"PTD\", 'L{$this->line} P{$m}'!E1:E200, 0))),0)";


            $opprej = "=INDEX('L{$this->line} P{$m}'!S1:S200, MATCH(\"PTD\", 'L{$this->line} P{$m}'!E1:E200, 0))";
            $oppqa = "=INDEX('L{$this->line} P{$m}'!T1:T200, MATCH(\"PTD\", 'L{$this->line} P{$m}'!E1:E200, 0))";
            $opprejper = "=IFERROR(VALUE(INDEX('L{$this->line} P{$m}'!U1:U200, MATCH(\"PTD\", 'L{$this->line} P{$m}'!E1:E200, 0))),0)";

            $ldperej = "=INDEX('L{$this->line} P{$m}'!X1:X200, MATCH(\"PTD\", 'L{$this->line} P{$m}'!E1:E200, 0))";
            $ldpeqa = "=INDEX('L{$this->line} P{$m}'!Y1:Y200, MATCH(\"PTD\", 'L{$this->line} P{$m}'!E1:E200, 0))";
            $ldperejper = "=IFERROR(VALUE(INDEX('L{$this->line} P{$m}'!Z1:Z200, MATCH(\"PTD\", 'L{$this->line} P{$m}'!E1:E200, 0))),0)";

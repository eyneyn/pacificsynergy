...
class MTDLineSummaryExport implements WithHeadings, WithEvents, WithCustomStartCell, WithStyles, WithCharts, WithTitle
{
    // Class properties
...
private function createMTDTitleBlock(Worksheet $sheet, int $startRow, int $endRow): void
{
    ...
    $staticCols = ['B','C','D','E','F','G','H','I','K','L','M','O','P','Q'];
    $dynamicCols = [];

    $oplStart = 19; // column S
    foreach ($oplTypes as $i => $name) {
        $dynamicCols[] = $this->colNameFromIndex($oplStart + $i);
    }
    $eplStart = $oplStart + count($oplTypes) + 2;
    foreach ($eplTypes as $j => $name) {
        $dynamicCols[] = $this->colNameFromIndex($eplStart + $j);
    }

    $allCols = array_merge($staticCols, $dynamicCols);
    ...
    foreach ($weekLabels as $i => $label) {
        ...
        $targetRows = [];
        if ($label !== 'PTD') {
            $targetRows = $this->weeklyData[$i]['excel_rows'] ?? [];
        } else {
            $targetRows = collect($this->weeklyData)->pluck('excel_rows')->flatten()->toArray();
        }
        ...
        if (count($targetRows)) {
            ...
            // LE Avg
            $sheet->setCellValue("F{$rowNum}", "=IFERROR(AVERAGE(F{$minRow}:F{$maxRow}),\"\")");
            $sheet->getStyle("F{$rowNum}")->getNumberFormat()->setFormatCode('0.00%');

            // Totals
            $sheet->setCellValue("K{$rowNum}", "=IFERROR(SUM(K{$minRow}:K{$maxRow}),\"\")");
            $sheet->setCellValue("L{$rowNum}", "=IFERROR(SUM(L{$minRow}:L{$maxRow}),\"\")");
            $sheet->setCellValue("M{$rowNum}", "=IFERROR(SUM(M{$minRow}:M{$maxRow}),\"\")");

            // DT%
            $sheet->setCellValue("O{$rowNum}", "=IFERROR(1-AVERAGE(F{$minRow}:F{$maxRow}),\"\")");
            $sheet->getStyle("O{$rowNum}")->getNumberFormat()->setFormatCode('0.00%');

            // OPL% & EPL% (G/H)
            $sheet->setCellValue("G{$rowNum}", "=IFERROR(L{$rowNum}/K{$rowNum}*O{$rowNum},\"\")");
            $sheet->setCellValue("H{$rowNum}", "=IFERROR(M{$rowNum}/K{$rowNum}*O{$rowNum},\"\")");
            $sheet->getStyle("G{$rowNum}:H{$rowNum}")->getNumberFormat()->setFormatCode('0.00%');

            // Percent Impact (P/Q)
            $sheet->setCellValue("P{$rowNum}", "=IFERROR(L{$rowNum}/K{$rowNum}*O{$rowNum},\"\")");
            $sheet->setCellValue("Q{$rowNum}", "=IFERROR(M{$rowNum}/K{$rowNum}*O{$rowNum},\"\")");
            $sheet->getStyle("P{$rowNum}:Q{$rowNum}")->getNumberFormat()->setFormatCode('0.00%');

            // Per-category OPL
            foreach ($oplTypes as $j => $name) {
                $col = $this->colNameFromIndex($oplStart + $j);
                $sheet->setCellValue("{$col}{$rowNum}", "=IFERROR(SUM({$col}{$minRow}:{$col}{$maxRow}),\"\")");
            }

            // Per-category EPL
            foreach ($eplTypes as $k => $name) {
                $col = $this->colNameFromIndex($eplStart + $k);
                $sheet->setCellValue("{$col}{$rowNum}", "=IFERROR(SUM({$col}{$minRow}:{$col}{$maxRow}),\"\")");
            }

// Mirror PTD LE into C6
if ($label === 'PTD') {
    $sheet->setCellValue("C6", "=F{$rowNum}");
    $sheet->getStyle("C6")->getNumberFormat()->setFormatCode('0.00%');

    // ✅ Apply style: bg D8E4BC, bold, centered
    $sheet->getStyle("C6")->applyFromArray([
        ...
        'fill' => [
            ...
            // No data → fill blanks
            foreach (array_merge(['F','G','H','K','L','M','O','P','Q'], $dynamicCols) as $col) {
...
/**
 * Build weekly summary data by grouping daily rows already prepared.
 *
 * @return array
 */
private function prepareWeeklyData(): array
{
    $weeklyRows = [];

    // Group daily data (already prepared with excelRow numbers)
    $grouped = collect($this->data)->groupBy(function ($row) {
        return Carbon::parse($row['productionDate'])->isoWeek();
    });

    foreach ($grouped as $weekNum => $records) {
        $excelRows = collect($records)->pluck('excelRow')->toArray();

        $weeklyRows[] = [
            'week'       => $weekNum,
            'excel_rows' => $excelRows, // ✅ store all excel row numbers
        ];
    }

    $this->weeklyData = $weeklyRows;
    return $weeklyRows;
}
...
private function prepareData(): array
{
    ...
    // Query directly from analytics table
    $dailyData = LineEfficiencyAnalytics::where('line', $this->line)
    ...
    foreach ($dailyData as $date => $records) {
        ...
        $oplTotals = $records->where('downtime_type','OPL')
                            ->groupBy('category')
                            ->map(fn($g) => $g->sum('minutes'))
                            ->toArray();

        $eplTotals = $records->where('downtime_type','EPL')
                            ->groupBy('category')
                            ->map(fn($g) => $g->sum('minutes'))
                            ->toArray();
        ...
        $rowData = [
            ...
            'sku'             => $first->sku ?? 'No Run',
            'line_efficiency' => $first->line_efficiency ?? 0,
        ];

$rows[] = $this->formatReportRow(
    $rowData,
    $dayCount++,
    $oplTotals,
    $eplTotals,
...
/**
 * Format a single production report (daily or weekly) into a table row
 *
 * @param mixed $report   Either ProductionReport | LineEfficiencyAnalytics | array
 * @param int   $dayNumber
 * @param array $oplTotals
 * @param array $eplTotals
 * @param int   $excelRow
 * @return array
 */
private function formatReportRow(
    $report,              // 👈 removed type-hint, allow array or object
    int $dayNumber,
    array $oplTotals = [],
    array $eplTotals = [],
    int $excelRow = 0
): array {
    // --- Normalize values depending on input type ---
    $productionDate = Carbon::parse(
        is_array($report) ? $report['production_date'] : $report->production_date
    )->format('F j, Y');

    $sku = is_array($report)
        ? ($report['sku'] ?? 'No Run')
        : ($report->standard->description ?? $report->sku ?? 'No Run');

    $size = is_array($report)
        ? ($report['size'] ?? 0)
        : ($report->standard->size ?? 0);

    $line_efficiency_raw = is_array($report)
        ? ($report['line_efficiency'] ?? 0)
        : ($report->line_efficiency ?? 0);

    $targetefficiency   = '80.00%';
    $line_efficiency    = $line_efficiency_raw / 100;

    // --- Downtime totals ---
    $oplMin   = array_sum($oplTotals);
    $eplMin   = array_sum($eplTotals);
    $total_DT = $oplMin + $eplMin;

    $totalper_DT = number_format(100 - $line_efficiency_raw, 2) . '%';
    $oplPerValue = $total_DT > 0 ? ($oplMin / $total_DT) * (100 - $line_efficiency_raw) : 0;
    $oplPer      = number_format($oplPerValue, 2) . '%';
    $epPerValue  = $total_DT > 0 ? ($eplMin / $total_DT) * (100 - $line_efficiency_raw) : 0;
    $epPer       = number_format($epPerValue, 2) . '%';
    ...
    $oplData = [];
    foreach ($oplTypes as $name) {
        $oplData[] = $oplTotals[$name] ?? 0;
    }

    $eplData = [];
    foreach ($eplTypes as $name) {
        $eplData[] = $eplTotals[$name] ?? 0;
    }

    // --- Return normalized row ---
    return [
        'dayNumber'        => $dayNumber,   // Column A
        'productionDate'   => $productionDate,
        'sku'              => $sku,
        'size'             => $size ?: '',
        'targetefficiency' => $targetefficiency,
        'line_efficiency'  => $line_efficiency,
        'oplPercent'       => "=IFERROR(P{$excelRow},0)",
        'eplPercent'       => "=IFERROR(Q{$excelRow},0)",
        'topDT'            => $topDT,
        'empty1'           => '',
        'totalDT'          => "=IFERROR(SUM(L{$excelRow},M{$excelRow}),0)",
        'oplMin'           => $oplMin,
        'eplMin'           => $eplMin,
        'empty2'           => '',
        'totalDTPercent'   => "=IFERROR(TEXT((100 - (F{$excelRow}*100))/100,\"0%\"),\"0%\")",
        'oplPercentCalc'   => "=IFERROR((L{$excelRow}/K{$excelRow})*(O{$excelRow}),0)",
        'eplPercentCalc'   => "=IFERROR((M{$excelRow}/K{$excelRow})*(O{$excelRow}),0)",
        'empty3'           => '',
        'oplData'          => $oplData,
        'eplData'          => $eplData,
        'excelRow'         => $excelRow,   // ✅ metadata only
    ];
}
    ...
    $lineEfficiencySeries->setPointSize(5);
    ...
    $titleRun = $richTitle->createTextRun("P{$this->month} LE % - Line {$this->line}");
    ...
    private function addChartFooterTable(
        ...
        // Data columns
        $indicators = [
            ['Target LE, %', 'E', 'percent'],  // match chart
            ['LE, %',        'F', 'percent'],   // match chart
            ['OPL, %',                 'G', 'percent'],
            ['EPL, %',                 'H', 'percent']
        ];

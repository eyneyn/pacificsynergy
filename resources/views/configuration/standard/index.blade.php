...
@section('content')
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between">
        ...
        <form method="GET" action="{{ route('configuration.standard.index') }}">
    ...
    <table class="w-full mt-4 text-sm text-left rtl:text-right border border-[#E5E7EB] border-collapse">
        ...
        <tbody>
            @forelse ($standards as $standard)
                <tr
                    onclick="window.location='{{ route('configuration.standard.view', $standard) }}'"
                ...
                >
                    ...
                    <td class="px-6 py-2 border border-[#d9d9d9] text-gray-600 text-center">{{ $standard->size }}</td>
                    <td class="px-6 py-2 border border-[#d9d9d9] text-gray-600 text-center">{{ $standard->bottles_per_case }}</td>
                    <td class="px-6 py-2 border border-[#d9d9d9] text-gray-600 text-center">{{ $standard->mat_no }}</td>
                    <td class="px-6 py-2 border border-[#d9d9d9] text-gray-600 text-center">{{ ucfirst($standard->group) }}</td>
                    <td class="px-6 py-2 border border-[#d9d9d9] text-gray-600 text-center">{{ $standard->preform_weight }}</td>
                    <td class="px-6 py-2 border border-[#d9d9d9] text-gray-600 text-center">{{ $standard->ldpe_size }}</td>
                </tr>
            @empty
                <tr>

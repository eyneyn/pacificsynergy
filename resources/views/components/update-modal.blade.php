<!-- Update Confirmation Modal -->
<div id="update-confirmation-modal" tabindex="-1"
     class="hidden fixed inset-0 z-50 flex justify-center items-center w-full p-4 bg-black/50 backdrop-blur-sm">
  <div class="relative w-full max-w-sm max-h-[90vh]">
    <div class="bg-white rounded-xl shadow-lg border border-gray-300 p-6">
      <h2 class="text-lg font-semibold text-[#323B76] mb-4">Confirm Update</h2>
      <p class="text-sm text-gray-600 mb-6">
        Are you sure you want to update this Production Report?
      </p>
      <div class="flex justify-end gap-3">
        <button type="button" id="cancel-update-btn"
                class="text-[#323B76] bg-white border border-[#323B76] hover:bg-gray-50 font-medium rounded-lg text-sm px-4 py-2.5">
          Cancel
        </button>
        <button type="submit" id="confirm-update-btn"
                class="text-white bg-[#323B76] hover:bg-[#2d326b] font-medium rounded-lg text-sm px-4 py-2.5">
          Yes, Update
        </button>
      </div>
    </div>
  </div>
</div>
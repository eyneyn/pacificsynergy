...
@section('content')
<div class="container mx-auto px-4" x-data="{ tab: 'info' }">
    <!-- Header -->
    <div class="mb-4">
    ...
    <div class="border-t border-b border-gray-200 mb-4 mx-auto grid grid-cols-1 lg:grid-cols-4 gap-6">
        ...
        <div class="col-span-1 p-8 text-center">
            ...
            <p class="text-sm text-blue-600 hover:underline cursor-pointer">Change Profile Picture</p>
        ...
        <div class="col-span-3 p-8">
            ...
            <div x-show="tab === 'account'" x-cloak>
                ...
                <div class="grid grid-cols-1 md:grid-cols-1 gap-6">
                    <div>
                        ...
                        <input type="email" name="email" value="{{ old('email', $user->email) }}"
                            class="w-1/2 border border-gray-300 px-3 py-2 text-sm focus:border-blue-500 focus:shadow-lg focus:outline-none" />
                    </div>
                </div>
                ...
                <div class="mt-6">
                    ...
                    <form method="POST" action="{{ route('password.email') }}" class="flex items-center gap-4">
                        @csrf
                        {{-- Always send the latest email typed above --}}
                        <input type="hidden" id="reset-email" name="email" value="{{ old('email', $user->email) }}">

                        <button type="submit"
                            onclick="document.getElementById('reset-email').value = document.querySelector('input[name=email]').value"
                            class="px-3 py-2 text-sm font-medium text-white bg-[#323B76] hover:bg-[#444d90] transition">
                            Send Reset Link
                        </button>

                        <p class="text-sm font-semibold text-[#23527c]">

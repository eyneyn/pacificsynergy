...
<div class="w-full flex items-center justify-center mb-6">
    <div class="w-full border-t border-[#fffffb]"></div>
</div>
...
<div>
    ...
    <form action="{{ route('analytics.material.export_excel_annual') }}" method="GET" class="inline-block">
...
<div class="w-full flex flex-col xl:flex-row gap-4 mb-4">
    <!-- Chart + Table Section -->
    <div class="w-full xl:w-3/4 bg-white rounded-sm border border-gray-200 p-6 shadow-md space-y-5 transition-all duration-300 hover:shadow-xl hover:border-[#fffffb] flex flex-col">
        ...
        <div class="overflow-x-auto mt-4">
                ...
                <thead class="bg-[#f1f5f9]  text-center">
                    <tr>
                        ...
                        @foreach (range(1,12) as $month)
                            <th class="border border-gray-300 px-2 py-1 text-right whitespace-nowrap">{{ $month }}</th>
                        @endforeach
                    </tr>
                </thead>
            ...
            <span class="h-[2px] w-4 bg-[#376faa]"></span>
                ...
                </td>                        @foreach ($monthlyProduction as $item)
            ...
            <span class="h-[2px] w-4 bg-[#7F7F7F]"></span>
                    ...
                    PREFORMS % Rejects
                </td>                        @foreach ($preformRejectRates as $item)
                            <td class="border border-gray-300 px-2 py-1 text-right">{{ $item }}</td>
                        @endforeach
                    </tr>
                    <tr>
            ...
            <span class="h-[2px] w-4 bg-[#254061]"></span>
                    ...
                    CAPS % Rejects
                </td>                        @foreach ($capsRejectRates as $item)
                            <td class="border border-gray-300 px-2 py-1 text-right">{{ $item }}</td>
                        @endforeach
                    </tr>
                    <tr>
            ...
            <span class="h-[2px] w-4 bg-[#77933C]"></span>
                    ...
                    OPP LABELS % Rejects
                </td>                        @foreach ($oppRejectRates as $item)
                            <td class="border border-gray-300 px-2 py-1 text-right">{{ $item }}</td>
                        @endforeach
                    </tr>
                    <tr>
...
<span class="h-[2px] w-4 bg-[#984807]/70"></span>
                ...
                </td>                        @foreach ($ldpeRejectRates as $item)
                            <td class="border border-gray-300 px-2 py-1 text-right">{{ $item }}</td>
                        @endforeach
                    </tr>
                </tbody>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 h-full">
            ...
            <div class="col-span-1 sm:col-span-2 bg-white hover:bg-[#e5f4ff] rounded-xl shadow border border-gray-200 p-3 h-28 flex items-center justify-center">
                <div class="text-center">
                    ...
                    <p class="text-lg font-semibold text-[#23527c]">{{ $materialEfficiencyRate }}</p>
                </div>
            </div>
            @php
                $cards = [
                    ['title' => 'Preform', 'value' => $preformTotalRate ?? '0.00%', 'color' => '#808080'],
                    ['title' => 'Caps', 'value' => $capsTotalRate ?? '0.00%', 'color' => '#16365C'],
                    ['title' => 'OPP', 'value' => $oppTotalRate ?? '0.00%', 'color' => '#4F6228'],
                    ['title' => 'LDPE', 'value' => $ldpeTotalRate ?? '0.00%', 'color' => '#963634'],
                ];
            @endphp
...
<div x-data="{ showMonth: true }" class="w-full mb-4">
    <!-- Toggle Button -->
    <div class="flex items-center justify-between cursor-pointer bg-white rounded-sm border border-gray-200 p-4 shadow-md hover:shadow-xl hover:border-[#fffffb]" @click="showMonth = !showMonth">
    ...
    <div x-show="showMonth" x-transition>
        <div class="bg-white rounded-sm border border-gray-200 p-6 shadow-md space-y-4 mt-2 overflow-x-auto">
            @php
...
<table class="w-full text-xs text-left border border-[#fffffb] border-collapse">
    <thead class="text-[8px] text-white uppercase bg-[#0070C0] whitespace-nowrap">
        ...
        {{-- Header row --}}
        <tr>
            <th class="p-2 border border-[#fffffb] text-center w-[120px] whitespace-nowrap">Period</th>
            <th class="p-2 border border-[#fffffb] text-center w-[120px] whitespace-nowrap">Month</th>
            <th class="p-2 border border-[#fffffb] text-center w-[160px] whitespace-nowrap">Target Mat'l Efficiency, %</th>
            <th class="p-2 border border-[#fffffb] text-center w-[130px] whitespace-nowrap">Production Output</th>
...
@foreach ($months as $i => $month)
    @php $m = $i + 1; @endphp
    <tr class="group text-[10px] whitespace-nowrap">
        ...
        {{-- ✅ Production Output (sum per month) --}}
        <td class="p-2 border border-[#fffffb] bg-[#F2F2F2] text-center">
            {{ number_format($monthlyProduction[$m] ?? 0) }}
        </td>
        ...
        <td class="border border-[#F2F2F2] p-2 text-center {{ rejectBgClass($preformRejectRates[$m] ?? '0.00%') }}">
        ...
        <td class="border border-[#F2F2F2] p-2 text-center {{ rejectBgClass($capsRejectRates[$m] ?? '0.00%') }}">
        ...
        <td class="border border-[#F2F2F2] p-2 text-center {{ rejectBgClass($oppRejectRates[$m] ?? '0.00%') }}">
        ...
        <td class="border border-[#F2F2F2] p-2 text-center {{ rejectBgClass($ldpeRejectRates[$m] ?? '0.00%') }}">

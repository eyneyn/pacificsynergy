...
<div class="flex flex-col md:flex-row gap-2 mb-8">
    ...
    <form action="{{ route('analytics.line.export_excel') }}" method="GET" class="inline-block">
    ...
    <div x-data="{ activeTab: 'overview' }" class="w-full mb-6">
                {{-- Tabs navigation --}}
        @include('analytics.line.partial.tabs-nav')

        {{-- Overview --}}
        @include('analytics.line.partial.tab-overview', [
            'labelsDays'   => $labelsDays,
            'monthNumber'  => $monthNumber,
            'year'         => $year,
            'line'         => $line,
            'eplPercent'   => $eplPercent ?? null,
        ])

        {{-- OPL Analysis --}}
        @include('analytics.line.partial.tab-opl', [
            'oplCategories' => $oplCategories,
            'oplData'       => $oplData,
        ])

        {{-- EPL Analysis --}}
        @include('analytics.line.partial.tab-epl', [
            'eplCategories' => $eplCategories,
            'eplData'       => $eplData,
        ])

        {{-- Production Report --}}
        @include('analytics.line.partial.tab-production', [
            'dailyRows'  => $dailyRows,
